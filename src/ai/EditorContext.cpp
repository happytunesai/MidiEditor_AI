...
QString EditorContext::ffxivContext()
{
    ...
    return QStringLiteral(
        ...
        "Each track maps to its corresponding channel: T0→CH0, T1→CH1, T2→CH2, etc.\n"
        "Exception: Percussion tracks can all share channel 9 (the GM drumkit channel).\n"
        "\n"
        "At tick 0, EVERY track must have program_change events for ALL used channels\n"
        "to define the instrument mapping. This lets MIDI players like MidiBard2 know\n"
        "which instrument belongs to which channel.\n"
        "\n"
        "Example — 8-track Octett with guitar switches:\n"
        "  T0 Flute         → CH0 (program 73)\n"
        "  T1 Trumpet        → CH1 (program 56)\n"
        "  T2 Violin         → CH2 (program 40)\n"
        "  T3 ElectricGuitarClean → CH3 (program 27)\n"
        "  T4 Lute            → CH4 (program 24)\n"
        "  T5 Snare Drum      → CH9 (program 115)\n"
        "  T6 Cymbal          → CH9 (program 127)\n"
        "  T7 Bass Drum       → CH9 (program 117)\n"
        "  Additional guitar channels (no extra tracks!):\n"
        "  CH5 ElectricGuitarMuted (28), CH6 ElectricGuitarOverdriven (29),\n"
        "  CH7 ElectricGuitarPowerChords (30), CH8 ElectricGuitarSpecial (31)\n"
        "\n"
        ...
        "\n"
        "### Guitar Switches\n"
        "Guitar switches are the ONLY case where channels matter beyond the track name.\n"
        "5 guitar variants can share a track. Each variant needs its own channel with\n"
        "program_change at tick 0. Switch between variants by changing the per-note channel.\n"
        "When ANY guitar is used, create ALL 5 guitar channels (no extra tracks needed).\n"
        "IMPORTANT: Every track must have program_change events for ALL used channels at\n"
        "tick 0 — not just its own channel.\n"
...
QString EditorContext::ffxivContextCompact()
{
    ...
    return QStringLiteral(
        ...
        "- Track N maps to Channel N: T0→CH0, T1→CH1, etc. Drums can share CH9.\n"
        "- ALL tracks must have program_change events for ALL used channels at tick 0.\n"
        "- Guitar switches: each variant needs its own channel (no extra tracks). Switch by changing note channel.\n"
        "\n"
        ...
        "\n"
        "### Guitar Switches\n"
        "5 guitar variants can share a track. Each variant needs all channels\n"
